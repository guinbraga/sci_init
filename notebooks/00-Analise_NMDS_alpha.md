...
<!-- #region id="o8v8w4nGdP3n" -->
# Imports

<!-- #endregion -->

```python colab={"base_uri": "https://localhost:8080/"} id="nh5Pzk9yhNS7" outputId="e61902ef-d7d1-40c6-b965-a6a9c94b892e"
pip install scikit-bio

```

```python colab={"base_uri": "https://localhost:8080/", "height": 944} id="VsznwQJOwC3c" outputId="70786185-ce99-4919-dfc2-a6a25979f9cf"
...
<!-- #region id="OQBG01N0dL_I" -->
# Loading

<!-- #endregion -->

```python id="RENy0EjPdLMj"
...
```python id="xFM_FeKdf8Mn"
meta_all_file = "../drive/01_metadata_all.csv"
meta_all_csv = pd.read_csv(meta_all_file, index_col=0)
HRT_metadata_ML_all  = meta_all_csv.copy()
```

```python colab={"base_uri": "https://localhost:8080/", "height": 1000} id="1C_UYBvMFCtJ" outputId="c9aec8b2-136c-43d6-af41-635dd336cdaf"
HRT_metadata_ML_all.head()
```

```python colab={"base_uri": "https://localhost:8080/"} id="yZg-R7NydueV" outputId="373e528c-26ee-4f89-b315-5bd0fc4389be"
asv_all_file = "../drive/Bettle_experiments/01_map_complete_relative_abundance_table.tsv"
...
```python colab={"base_uri": "https://localhost:8080/", "height": 270} id="1oiVFkSTIWIJ" outputId="1d8aa932-7ff2-4c6b-f225-0ccb26ab98fa"
asv_all_csv.head()
```

```python id="ebNbxoeVet9B"
# Salvando em arquivos CSV
asv_all_csv.to_csv("../outputs/nb_out/05_asv_bin_ML_ra.csv")
HRT_asv_ML_all = asv_all_csv.copy()

```

<!-- #region id="z0kZOEC8KyPk" -->
...
```python id="O4WU3XNdfzu-"
# Adicionar SampleID como coluna temporária
HRT_asv_ML_all.reset_index(names=['SampleID'], inplace=True)
HRT_asv_ML_all['SampleID'] = HRT_asv_ML_all['SampleID'].astype(str).str.strip()
```

```python id="YS0da7TWJRmX"
# Adicionar SampleID como coluna temporária
HRT_metadata_ML_all.reset_index(inplace=True)
HRT_metadata_ML_all['SampleID'] = HRT_metadata_ML_all['SampleID'].astype(str).str.strip()
```

```python colab={"base_uri": "https://localhost:8080/", "height": 238} id="GQCpv1tTI6ZJ" outputId="b434fe24-212a-48bc-a988-d848101f870a"
HRT_metadata_ML_all.head()
```

```python id="f5yIn2ZLf0Pm"
...
```python id="xLqbWEaVJg3Z"
# Transformar SampleID de volta em índice e remover a coluna SampleID
HRT_metadata_ML_all.set_index('SampleID', inplace=True)
HRT_metadata_ML_all.index.name = None
```

```python colab={"base_uri": "https://localhost:8080/", "height": 1000} id="sMLRnXjCJnH8" outputId="a04445b6-fc43-4a10-d791-f7cedc6ba789"
HRT_ML_reactor.head()
```

```python colab={"base_uri": "https://localhost:8080/", "height": 206} id="Bkrq1SOjJ1r2" outputId="7342df81-7adc-40fd-a5a6-66b387beac32"
HRT_metadata_ML_all.head()
```

```python id="YCqxgt2rg73q"
...
```python colab={"base_uri": "https://localhost:8080/", "height": 458} id="Vte0BzUmhJxu" outputId="0ad96406-3c2e-409a-97b1-123b5fddf283"
HRT_ML_reactor.dtypes

```

```python
print(HRT_metadata_ML_all['Category'].unique())
print(HRT_metadata_ML_all['Experiment'].unique())
```

```python id="eeGmcUoHLVvY"
...
<!-- #region id="lP6TUPHddsGP" -->
# Anlyses

<!-- #endregion -->

```python colab={"base_uri": "https://localhost:8080/"} id="3FxoYqoFKKV8" outputId="5a504da8-5abc-4334-9889-0f27a328dd15"
# Cada linha representa uma amostra e as colunas são OTUs
data = HRT_asv_ML_all.copy()
data.shape
```

```python colab={"base_uri": "https://localhost:8080/", "height": 1000} id="A03DOAyy3mwe" outputId="75cef4fb-5df8-41b6-bae3-c80d135d9984"
metadata = HRT_metadata_ML_all.loc[:, ['Gut compartiment', 'Experiment', 'Category']].copy()
metadata
```

```python colab={"base_uri": "https://localhost:8080/"} id="SqItK-VxMpEU" outputId="b03cb85a-10ea-4d41-b82c-ff1dc74cbab0"
metadata.shape
```

```python colab={"base_uri": "https://localhost:8080/"} id="HTtwU8RT5xCE" outputId="aceba065-3483-41cf-c485-4879c31b88f0"
print(data.dtypes)

```

```python colab={"base_uri": "https://localhost:8080/", "height": 1000} id="tws7m-SMx2s2" outputId="68eb56b0-72a8-44cf-d665-f71c2ad4896a"
metadata['Category']
```

```python colab={"base_uri": "https://localhost:8080/"} id="8HvZnEMkxrhs" outputId="fe9c7ff5-422e-44ad-c344-8c233420d777"
metadata.columns
```

```python colab={"base_uri": "https://localhost:8080/"} id="Hs_DH1kQEgHP" outputId="33077d3f-22a1-478a-de82-2c3220de4f3a"
data_numeric.shape
```

```python colab={"base_uri": "https://localhost:8080/", "height": 1000} id="fpsSxkjoc5V3" outputId="bdc0d1f3-bad2-461d-b9ae-b04eb0cde5d3"
import numpy as np
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
