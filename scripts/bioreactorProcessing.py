...
rel_file = "../drive/Bettle_experiments/01_map_complete_relative_abundance_table.tsv"
...
full_abs = pd.merge(
    absolute_hits, meta_df[["Experiment"]], left_index=True, right_index=True
)
full_rel_raw = pd.merge(
    rel_abundance, meta_df[["Experiment"]], left_index=True, right_index=True
)
full_rel_calc = pd.merge(
    rel_from_abs, meta_df[["Experiment"]], left_index=True, right_index=True
)
...
train_abs, test_abs = train_test_split(
    full_abs, test_size=0.25, stratify=full_abs["y"], random_state=42
)
